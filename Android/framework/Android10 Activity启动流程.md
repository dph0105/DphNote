...
##### 2.5.1.4 通过ActivityThread暂停Activity
...
##### 2.5.1.5 通过Instrumentation分发onPause事件
...
当Handler收到超时消息时，最终会调用activityPausedLocked方法，这个方法我们在启动新Activity中分析

#### 2.5.2 启动新Activity

#### 2.5.2.1 app进程通知ATMS已经暂停Activity
...
completePauseLocked首先设置mPausingActivity的状态为PAUSED，然后将它的值设为null。由于这里的方法参数resumeNext为ture，最后又重新来到`RootActivityContainer.resumeFocusedStacksTopActivities`方法中，在2.7.1暂停Activity一节中，我们知道最终调用`resumeTopActivityInnerLocked`，然后调用`startPausingLocked`方法开始暂停Activity。

而这一次重新来到这里，由于在`completePauseLocked`函数中，通过`prev.setState(PAUSED, "completePausedLocked")`将mResumedActivity设置为null，所以不会进入startPausingLocked中，具体看下面分析
...
当通过Binder机制将事务发送到app进程时，app进程会通过Handler发送消息到主线程，然后通过TransactionExecutor来处理。
