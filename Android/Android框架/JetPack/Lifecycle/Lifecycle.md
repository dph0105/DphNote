...
```java
    ...
    public void addObserver(@NonNull LifecycleObserver observer) {
         //1.创建ObserverWithState，并以LifecycleObserver为key，
        // ObserverWithState为value传入map中记录
...
##### 3.1.1.2 转化LifecycleObserver为LifecycleEventObserver
...
```
我们generatedConstructor会根据传入的类型，得到编译时，自动生成的带有我们自己创建的Observer的类名+_LifecycleAdapter后缀的一个GeneraterAdatper的子类的构造方法。当然，如果没有这个类，则返回null。
...
###### 2. 创建ReflectiveGenericLifecycleObserver
...
LifecycleEventObserver是事件的真正的接收者，若我们传入的Observer实现的是LifecycleObserver，那么ObserverWithState中则会得到一个`ReflectiveGenericLifecycleObserver`，若我们直接实现LifecycleEventObserver，则ObserverWithState中的就是我们自己实现的。

ReflectiveGenericLifecycleObserver通过注解找到我们自定义的方法，并缓存方法，当调用时，则通过反射调用响应的方法。

#### 3.1.2

但是addObserver(LifecycleObserver observer)并没有结束：
...
```java
    ...
    public void addObserver(@NonNull LifecycleObserver observer) {
        ...
        boolean isReentrance = mAddingObserverCounter != 0 || mHandlingEvent;
        //2.添加后Observer后，先更新到当前状态
        //2.1计算出组件当前的状态
        State targetState = calculateTargetState(observer);
        mAddingObserverCounter++;
        //2.2更新ObserverWithState的状态到最新
        while ((statefulObserver.mState.compareTo(targetState) < 0
                && mObserverMap.contains(observer))) {
            pushParentState(statefulObserver.mState);
            statefulObserver.dispatchEvent(lifecycleOwner, upEvent(statefulObserver.mState));
            popParentState();
            // 因为组件的状态可能在过程中改变，所以每次循环都要计算
            targetState = calculateTargetState(observer);
        }
