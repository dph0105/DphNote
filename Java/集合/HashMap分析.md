...
```java
    final Node<K,V>[] resize() {
        ...
        if (oldCap > 0) {
            ...
            //这种情况可能为构造方法为有参构造
            //如果没有初始化，那么设置新的数组容量为旧的threshold，
        ...
        //设置threshold为newThr
        //注：有参构造时，设置了threshold为2的幂，而这里第一次加载时，还是会设置为数组容量*负载因子
        threshold = newThr;
        //创建数组
        ...
        if (oldTab != null) {
            for (int j = 0; j < oldCap; ++j) {
                ...
                if ((e = oldTab[j]) != null) {
                    oldTab[j] = null;
                    if (e.next == null)
                        //如果数组j位置的元素不是链表，通过e.hash & (newCap - 1)计算新的位置
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        if ((p = tab[i = (n - 1) & hash]) == null)
            ...
            if (p.hash == hash &&
                ...
                //如果不是同一个Node，并且桶是红黑树结构，增加一个树节点
                e = ((TreeNode<K,V>)p).putTreeVal(this, tab, hash, key, value);
            else {
                //如果不是同一个Node，并且桶是链表结构，增加一个链表节点
                for (int binCount = 0; ; ++binCount) {
                    if ((e = p.next) == null) {
                        p.next = newNode(hash, key, value, null);
                        if (binCount >= TREEIFY_THRESHOLD - 1)
                            //如果链表的长度大于等于 7 ，转化为红黑树
                            treeifyBin(tab, hash);
                        break;
                    }
